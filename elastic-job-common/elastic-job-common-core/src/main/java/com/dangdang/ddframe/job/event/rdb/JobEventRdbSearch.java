...
public final class JobEventRdbSearch {
    ...
    private String buildDataSql(final String tableName, final Collection<String> tableFields, final Condition condition) {
        ...
        //增加对oracle的支持
        if( "oracle.jdbc.OracleDriver".equals(((BasicDataSource)dataSource).getDriverClassName())){
            sqlBuilder.append(selectSql).append(whereSql).append(orderSql);
            return  buildRowNum(sqlBuilder.toString(), condition.getPage(), condition.getPerPage());
        }else{
    ...
    /**
     * @author antz-H
     * @description  默认仅支持mysql，现增加对oracle的支持
     * @date 2018.12.5
     * @param sql
     * @param page
     * @param perPage
     * @return
     */
    private String buildRowNum(String sql,int page,int perPage){
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("SELECT * FROM (SELECT T.*, ROWNUM RN FROM (")
                .append(sql)
                .append(") T  ) WHERE RN BETWEEN ")
                .append((page-1)*perPage+1)
                .append(" and ")
                .append(page* perPage);
        return sqlBuilder.toString();
    }
